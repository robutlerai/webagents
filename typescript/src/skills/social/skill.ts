...
// ---------------------------------------------------------------------------
// Portal connectivity tools — REMOVED (M4).
//
// `PortalConnectSkill` (portal_register / portal_heartbeat / portal_deregister)
// and `PortalWSSkill` targeted endpoints that do not exist or reject their
// bodies: portal_register POSTed a shape the register route's schema always
// 400'd (and the route itself is gone), portal_deregister resolved into the
// agent skill-path catch-all and did nothing, portal_heartbeat POSTed a body
// to a body-less route with the WRONG principal and only when an LLM chose
// to call it, and PortalWSSkill spoke a `register` protocol the platform's
// /ws never handled — its frames were dropped on the floor.
//
// The real paths: `PortalConnectSkill`
// (src/skills/transport/portal-connect) for the reverse bridge, and `serve()`
// (src/server/node.ts) for own-URL serving, which also publishes the agent
// card and beats presence. Liveness is a transport concern, not a
// model-invoked tool.
// ---------------------------------------------------------------------------
